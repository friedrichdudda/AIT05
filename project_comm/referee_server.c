...
static ssize_t _register_player_handler(coap_pkt_t *pdu, uint8_t *buf, size_t len,
                                        coap_request_ctx_t *ctx);
static ssize_t _increment_player_count_handler(coap_pkt_t *pdu, uint8_t *buf, size_t len,
                                               coap_request_ctx_t *ctx);

/* CoAP resources. Must be sorted by path (ASCII order). */
static const coap_resource_t _resources[] = {
    { "/register_player", COAP_PUT, _register_player_handler, NULL },
    { "/increment_player_count", COAP_PUT, _increment_player_count_handler, NULL },
};
                                        ...
                                        coap_request_ctx_t *ctx)
{
    if (player_1 == NULL) {
        player_1 = *ctx->remote;
        /* TODO change LED color to yellow */
    }
...
static ssize_t _increment_player_count_handler(coap_pkt_t *pdu, uint8_t *buf, size_t len,
                                               coap_request_ctx_t *ctx)
{
    int player_number = atoi((char *)buf);

    switch (player_number) {
    case 1: {
        player_1_count++;
        /* TODO let LED blink in player_1 color */
        break;
    }
    case 2: {
        player_2_count++;
        /* TODO let LED blink in player_2 color */
        break;
    }
    }
